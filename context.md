...
| **V41.1** | 2026/09/20 | **国家官方气象机构链接 100% 精准直达与多城市气象数据纠偏 (Official Meteorological Agency Precise Deep-Links & Multi-City Calibration)**：<br>1. **新西兰 MetService 官方直达深度链接精准化**：彻底修正原阿卡罗阿、蒂卡波湖跳转至基督城大区的模糊链接，精准对接官方专属城镇预报页：<br> - **阿卡罗阿 (Akaroa)**：`https://www.metservice.com/towns-cities/locations/akaroa`；<br> - **蒂卡波湖 (Lake Tekapo)**：`https://www.metservice.com/towns-cities/locations/tekapo`；<br> - **库克山国家公园 (Mt Cook)**：保留 Alpine 高山国家公园气象专页 `https://www.metservice.com/mountains-and-parks/national-parks/aoraki-mount-cook`（含山谷大风与雪线冻结高度）；<br> - **城堡山 / 亚瑟隘口**：`https://www.metservice.com/mountains-and-parks/national-parks/arthurs-pass`；<br> - **基督城**：`https://www.metservice.com/towns-cities/locations/christchurch`；<br>2. **澳大利亚 BOM 气象局全线升级 HTTPS**：布里斯班与悉尼官方预报链接全面由 `http://` 升级为 `https://` 安全加密协议，避免移动端安全警告；<br>3. **Day 10 & Day 11 数据纠偏与返程北京气象直达**：<br> - **Day 10**：剔除旧版残留的广州气象标签，专注于悉尼全天行程；<br> - **Day 11**：纠正原广州气象标签，准确配置为出发地【悉尼 BOM】与目的地【北京 中国天气网 CMA 官方直达 `http://www.weather.com.cn/weather/101010100.shtml`】双城温差预警；<br>4. **气象气泡与穿衣弹窗文案优化**：将链接文字统一升级为带有城市名称的精准引导（如 `🏛️ MetService NZ（蒂卡波湖 官方精准预报）↗`），消除歧义；<br>5. **全端版本与离线缓存穿透**：全量静态资源请求字符串、品牌徽章与 Service Worker 缓存池版本升级至 `v41.1`。 |
| **V41.2** | 2026/09/20 | **MetService 精准路径实测纠正 & 移除 API 原始数据查验选项 (MetService Real-Path Correction & Weather Popover Simplification)**：<br>1. **MetService 官方链接精确落地**：根据用户实测反馈，校准并替换为 MetService 真实可达的精准路径：<br> - **阿卡罗阿 (Akaroa)**：更新为坎特伯雷大区二级路径 `https://www.metservice.com/towns-cities/regions/christchurch/locations/akaroa`；<br> - **蒂卡波湖 (Lake Tekapo)**：确认为 `https://www.metservice.com/towns-cities/locations/tekapo`；<br> - **库克山国家公园 (Mt Cook)**：更新为官方标准山地高山专页 `https://www.metservice.com/mountains-and-parks/national-parks/aoraki-mt-cook`（同步更正 Day 5 查验卡片内的外部链接）；<br> - **城堡山与基督城**：确认保持现有国家公园与城市专页；<br>2. **气象详情弹窗纯净化 (去除 API 查验入口)**：在天气详情浮层中彻底移除 `📡 Open-Meteo API 原始数据 ↗` 查验选项，仅保留唯一权威的官方国家气象机构精准直达链接，界面更为整洁直观；<br>3. **全端版本与离线缓存穿透**：全量静态资源请求字符串、品牌徽章与 Service Worker 缓存池版本升级至 `v41.2`。 |
| **V41.3** | 2026/09/20 | **天气与穿衣弹窗图层层级 (z-index) 物理隔离与防遮挡修复 (Weather Popovers Stacking Context Isolation & Hover Shielding)**：<br>1. **修复相邻卡片 Hover 遮挡 Bug**：根除原 `.day-card:hover` 与 `.day-card.has-active-popover` 共享相同 `z-index: 9999` 导致后续天数卡片在鼠标移入时因 DOM 顺序反向遮挡已展开天气弹窗的致命缺陷；<br>2. **分级图层层级架构确立**：<br> - 基础卡片：`z-index: 1`；<br> - 普通悬停/聚焦卡片（`.day-card:hover`）：轻微提升至 `z-index: 5`（仅用于微光阴影平滑呈现，绝不凌驾弹窗）；<br> - 展开天气/穿衣弹窗的卡片（`.day-card.has-active-popover` 及 `:has(...)`）：强制锁定最高层级 `z-index: 10000 !important;`；<br> - 对应卡片头部（`.day-header`）：提升至 `z-index: 10001 !important;`；<br> - 活跃气象包装器：`z-index: 10002 !important;`；<br> - 气象/穿衣浮层（`.weather-detail-popover` / `.weather-clothing-popover`）：`z-index: 10005 !important;`；<br> - 全局模态框（`.lightbox-modal`、汇率、清单）：提升至 `z-index: 11000`，全局逻辑严丝合缝；<br>3. **全端版本与离线缓存穿透**：全量静态资源请求字符串、品牌徽章与 Service Worker 缓存池版本升级至 `v41.3`。 |
